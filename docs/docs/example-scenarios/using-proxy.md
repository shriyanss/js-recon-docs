...
Note that the configuration that will be generated by using the [`proxy`](../modules/proxy.md) module will be used by the [`lazyload`](../modules/lazyload.md) module (and any other module that makes outbound HTTP requests) to make HTTP requests. The sole purpose of the [`proxy`](../modules/proxy.md) module is to generate configuration and manage the API gateways created.

## Generating config file

To send HTTP requests through the AWS API Gateway, the proxy configuration file must be created. Before running the given command, make sure that you have the environment variables set or have the keys with proper configurations to pass to the tool.
...
This will create a config file called `.proxy_config.json` in the current working directory. Now, this configuration can be used to rotate IP addresses.
...
:::warning
The config file, that is `.proxy_config.json`, has the access and secret key hardcoded in it (under the `aws` key). Make sure to keep it safe - just like your passwords
:::
...
- [Destroy a specific API gateway](../modules/proxy.md#destroy-an-api)
- [Destroy all API gateways](../modules/proxy.md#destroy-all-apis)

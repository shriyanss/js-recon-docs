...
| 27        | `lazyload`   | Invalid stagnation configuration. The `--stagnation-timein` value exceeds `--lazyload-timeout`, so stagnation monitoring could never trigger before the whole crawl times out.                                      |
| 28        | `report`     | `sj` binary not found. The `--sj` flag was set but the `sj` (swagger-jacker) binary is not present in `PATH`. Install it with `go install github.com/BishopFox/sj@latest`.                                          |

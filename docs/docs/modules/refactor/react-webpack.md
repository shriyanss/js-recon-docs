...
The `--collisions <file>` flag enables a second pass that classifies every numeric webpack module as either _library code_ (e.g. React, React-DOM, scheduler, `react/jsx-runtime`) or _user code_, and **drops the library modules entirely** from the output. Only `index.js` (the application entry) is written.
